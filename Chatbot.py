import subprocess
subprocess.check_call(['pip', 'install', '--quiet', 'neo4j', 'langchain-community', 'langchain-core', 'langchain-openai', 'langgraph'])
...
def get_response(query: str) -> dict:
    # You can also add any pre/post processing here if needed.
    result = langgraph.invoke({"question": query})
    # Depending on your output requirements, you might extract or format the result.
    return result